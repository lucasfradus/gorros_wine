# Identidad visual: meter el isologo real en la web — Plan

**Objetivo:** que el símbolo de la marca exista como vector propio y aparezca en
las pantallas donde el sitio se identifica, más los iconos de browser, iOS y PWA.

**Enfoque:** se traza el canal alfa de `public/logos/isologo.png` a SVG con un
generador propio (`scripts/_isotipo.mjs`), y el símbolo se consume desde un único
componente (`components/isotipo.tsx`) que elige el peso óptico según el tamaño.

**Sin worktree, a propósito:** `scripts/worktree.ps1` basa todo worktree en
`origin/main`, y `origin/main` no tiene los archivos sin commitear del árbol de
entonces (route groups `(store)`/`(admin)`, `lib/db`, `lib/auth`). Un worktree
...
`public/logos/` tiene el pack de marca, pero el sitio nunca lo usa. La marca en
pantalla es sólo el texto "GORROS WINE" en dorado, y `app/icon.svg` es un gorro
de cocinero **inventado** que no tiene nada que ver con la marca.
...
- [x] `npx tsc --noEmit`
- [x] `npm run build`
- [x] Probado a mano en `:3000` — age gate, nav, footer e ingreso al panel.
- [x] PR mergeado a `main` — [#1](https://github.com/lucasfradus/gorros_wine/pull/1),
      el 2026-08-24, junto con la primera etapa del backend, con la que esta
      iteración compartió árbol de trabajo.
- [x] Sin worktree que borrar: se trabajó sobre `main` a propósito, por el
      motivo del encabezado. La rama `feat/backend-admin` se borró al mergear.
